 ...
 * - Worst Case: @c O(N log N) (mitigated by median-of-three and 3-way partitioning)
 *
...
private:
    ...
    /**
     * @brief In-place insertion sort helper for small subranges.
     */
    template <typename T>
    void insertion_sort_range(T* arr, std::size_t low, std::size_t high) const {
     ...
     * and places it at <tt>arr[low]</tt>.
     */
    template <typename T> void median_of_three(T* arr, std::size_t low, std::size_t high) const {
        ...
        // Place the median at arr[low]
        std::swap(arr[low], arr[mid]);
    }

    /**
     * @brief Dijkstra 3-Way Fat Partitioning (Dutch National Flag).
     *
     * Partitions <tt>arr[low..high]</tt> into:
     * - <tt>arr[low..lt-1] < pivot</tt>
     * - <tt>arr[lt..gt] == pivot</tt>
     * - <tt>arr[gt+1..high] > pivot</tt>
     *
     ...
     *
     * Recurses on the smaller partition and iterates on the larger partition to strictly
     * guarantee @c O(log N) maximum stack depth.
     */
