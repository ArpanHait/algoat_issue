 ...
 * @brief Unboxes `PyComplexObject` into a `std::complex<float>` for 2D Morton Z-order curve
 * comparisons.
 */
struct PyComplexWrapper {
    ...
    bool operator<(const PyComplexWrapper& other) const {
        return algoat::numerics::MortonCompare{}(val, other.val);
    }
    bool operator==(const PyComplexWrapper& other) const {
...
struct PyFloatWrapper {
    ...
    explicit PyFloatWrapper(PyObject* o) : obj(o) {
        if (PyFloat_Check(o))
            val = PyFloat_AsDouble(o);
        else
            val = (double)PyLong_AsLongLong(o);
    }

    bool operator<(const PyFloatWrapper& other) const {
        return val < other.val;
    }
    bool operator<=(const PyFloatWrapper& other) const {
        return val <= other.val;
...
struct PyBigIntWrapper {
    ...
    bool operator<(const PyBigIntWrapper& other) const {
        if (is_small && other.is_small)
            return small_val < other.small_val;
